(() => {
  ...
  function sx(x) { return VIEW.originX + x * VIEW.unit; }
  function sy(y) { return VIEW.originY - y * VIEW.unit; }
  ...
  function drawGrid() {
    ...
    for (let x = LIMITS.minX; x <= LIMITS.maxX; x += 1) {
      ...
      const line = createSvg('line', {
        x1: xPos, y1: sy(LIMITS.maxY), x2: xPos, y2: sy(LIMITS.minY),
      ...
      if (x !== 0) {
        const text = createSvg('text', {
          x: xPos, y: sy(0) + 20, fill: 'var(--muted)',
    ...
    for (let y = LIMITS.minY; y <= LIMITS.maxY; y += 1) {
      const yPos = sy(y);
      const line = createSvg('line', {
        x1: sx(LIMITS.minX), y1: yPos, x2: sx(LIMITS.maxX), y2: yPos,
      ...
      if (y !== 0) {
        const text = createSvg('text', {
          x: sx(0) - 12, y: yPos + 4, fill: 'var(--muted)',
    ...
    const xAxis = createSvg('line', {
      x1: sx(LIMITS.minX), y1: sy(0), x2: sx(LIMITS.maxX) + 12, y2: sy(0),
      stroke: 'var(--axis)', 'stroke-width': 2
    });
    axisLayer.appendChild(xAxis);

    const xArrow = createSvg('path', {
      d: `M ${sx(LIMITS.maxX) + 12} ${sy(0)} l -10 -6 v 12 z`, fill: 'var(--axis)'
    });
    axisLayer.appendChild(xArrow);

    const yAxis = createSvg('line', {
      x1: sx(0), y1: sy(LIMITS.minY), x2: sx(0), y2: sy(LIMITS.maxY) - 12,
      stroke: 'var(--axis)', 'stroke-width': 2
    });
    axisLayer.appendChild(yAxis);

    const yArrow = createSvg('path', {
      d: `M ${sx(0)} ${sy(LIMITS.maxY) - 12} l -6 10 h 12 z`, fill: 'var(--axis)'
    });
    axisLayer.appendChild(yArrow);

    const xLabel = createSvg('text', {
      x: sx(LIMITS.maxX) + 20, y: sy(0) - 9, fill: 'var(--axis)', 'font-size': 14, 'font-weight': 800
    });
    xLabel.textContent = 'x';
    axisLayer.appendChild(xLabel);

    const yLabel = createSvg('text', {
      x: sx(0) + 10, y: sy(LIMITS.maxY) - 18, fill: 'var(--axis)', 'font-size': 14, 'font-weight': 800
    });
  ...
  function executeAction(action) {
    ...
    if (!pointInBounds(state.x, state.y)) {
      ...
      setStatus(`Passo ${pc + 1} de ${program.length}. Estado: (${formatNumber(state.x)}, ${formatNumber(state.y)}), ${formatNumber(normalizeAngle(state.angle))}°.`);
    }
  }
  ...
  function checkChallenge() {
    ...
    const stayedOnTarget = route.length >= 5 && route.every(onTargetPerimeter) && route.slice(1).every((point, index) => segmentOnTarget(route[index], point));
